...
std::string Response::cleanPath(std::string &path) {
...
void Response::performGET() {
    ...
    if (S_ISDIR(_pathStat.st_mode) == false) {
        ...
        if (index.empty() == false) { // has index file
            fileToBody(index);
        } else if (_location.getAutoIndexing() == true) { // auto-indexing on
...
void Response::performPOST() {
    ...
    const char *extension = Response::getMimeExtenstion(_request.getHeaderValueForKey("content-type"));
    if (extension == NULL) {
        throw 415;
    }
